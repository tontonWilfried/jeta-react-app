...
export default function CdiscountProducts() {
  ...
  const [showSellerCart, setShowSellerCart] = useState(false);
  const navigate = useNavigate();

  useEffect(() => {
    fetch('http://localhost:3001/api/cdiscount-products')
      .then(res => res.json())
      .then(data => {
        setProducts(data);
        setLoading(false);
      ...
      .catch(error => {
        console.error('Erreur lors du chargement des produits Cdiscount:', error);
        setLoading(false);
      });
  }, []);
  ...
  const handleRefresh = async () => {
    ...
    setProgress(0);
    setNotif('Démarrage du scraping Cdiscount...');

    try {
      const response = await fetch('http://localhost:3001/api/scrape-cdiscount', {
        method: 'POST',
      });

      if (response.ok) {
        const eventSource = new EventSource('http://localhost:3001/api/scrape-cdiscount/progress');
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <img
                src="https://upload.wikimedia.org/wikipedia/commons/thumb/8/8a/Cdiscount_logo.svg/1200px-Cdiscount_logo.svg.png"
                alt="Cdiscount"
                className="w-12 h-12 object-contain"
              />
              <div>
                <h1 className="text-2xl font-bold text-[#4FC3F7]">Produits Cdiscount</h1>
                <p className="text-gray-600">Électronique, informatique, téléphonie</p>
              </div>
            </div>
          </div>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-semibold transition-all ${
              refreshDisabled || refreshing
                ...
                : 'bg-[#E91E63] text-white hover:bg-[#E91E63]/90'
            }`}
          >
            ...
            {paginatedProducts.map((product, index) => (
              <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                <div className="aspect-w-3 aspect-h-2 bg-gray-100">
                  <img
                    src={product.imageUrl || 'https://via.placeholder.com/300x200?text=Cdiscount'}
                    alt={product.name}
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-xs text-gray-500">Cdiscount</span>
                  </div>
                  <button
